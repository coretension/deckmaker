package io.github.coretension.deckmaker.persistence;
...
public class DeckStorage {
    private static final String CONFIG_DIRECTORY = ".deckmaker";
    private static final String LEGACY_CONFIG_DIRECTORY = ".cardmaker";
    private static final String SETTINGS_FILE = "settings.json";
    private static final String TEMP_DECK_FILE = "temp_deck.json";
    ...
    public static AppSettings loadSettings() throws IOException {
        ...
        File legacyFile = getLegacyConfigFile(SETTINGS_FILE);
        if (legacyFile.exists()) {
            AppSettings settings = mapper.readValue(legacyFile, AppSettings.class);
    ...
    public static File getSettingsFile() {
        return getConfigFile(SETTINGS_FILE);
    }
    ...
    public static File getTempFile() {
        return getConfigFile(TEMP_DECK_FILE);
    }

    /**
     * Returns the temp deck file to use for recovery, including legacy CardMaker autosaves.
     */
    public static File getRecoveryTempFile() {
        File file = getTempFile();
        if (file.exists()) {
            return file;
        }
        File legacyFile = getLegacyConfigFile(TEMP_DECK_FILE);
        return legacyFile.exists() ? legacyFile : file;
    }

    /**
     * Deletes current and legacy temporary deck files after a successful save/load.
     */
    public static void deleteTempFiles() {
        deleteIfExists(getTempFile());
        deleteIfExists(getLegacyConfigFile(TEMP_DECK_FILE));
    }

    private static File getConfigFile(String filename) {
        String userHome = System.getProperty("user.home");
        Path path = Paths.get(userHome, CONFIG_DIRECTORY, filename);
        File file = path.toFile();
        ensureParentExists(file);
        return file;
    }

    private static File getLegacyConfigFile(String filename) {
        String userHome = System.getProperty("user.home");
        Path path = Paths.get(userHome, LEGACY_CONFIG_DIRECTORY, filename);
        return path.toFile();
    }
