...
### The Interface
- **Left Panel (Elements):** A tree view showing all the components on your card. You can reorder them by dragging or delete them using the **Delete** key.
...
- **Position (X, Y):** Control the placement of the element in millimeters, relative to its parent.
- **Name:** A descriptive label for identifying the element in the tree view.

### Text
- **Basic Text:** Static text or tags like `{{Name}}`.
- **Rich Text:** Use lightweight tags such as `<b>bold</b>`, `<i>italic</i>`, `<u>underline</u>`, `<br>`, and `<span style="color:#ff0000">colored text</span>`.
...
- **Preview Mode:** (`View > Preview Mode` or `Space`) Hides element selection highlights and the bleed guide (if Professional Mode is on) for a clean look at your final card design.
